...
import { clavesAvisos, leerAvisos, modificarAvisos, suscripcionValida, agregarSuscripcion } from './_avisos.js';
...
export default async function handler(req, res) {
  ...
  const { empleado, suscripcion, baja, comprobar } = req.body || {};
  if (!suscripcion || !suscripcionValida(suscripcion)) return res.status(400).json({ error: 'Suscripción inválida' });
  try {
    if (comprobar) {
      // Solo lectura: el móvil pregunta si su suscripción sigue registrada.
      const { avisos } = await leerAvisos();
      const existe = avisos.suscripciones.some((s) => s.endpoint === suscripcion.endpoint && s.empleado === empleado);
